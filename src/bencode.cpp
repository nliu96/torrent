...
std::map<std::string, Bencode> Bencode::getDict()
{
  return dict_;
}
...
std::string encode(Bencode in)
{
  ...
  case Bencode::kDict:
  {
    ...
    for (const auto &dictEntry : in.getDict())
    {
      ...
      dictStrings = dictStrings + length + ":" + dictEntry.first + encode(dictEntry.second);
    }
...
Bencode decode(std::vector<char>::iterator &begin, std::vector<char>::iterator &end)
{
